var itl = {
    ...
    ajaxform: function (url, formData, modal, callback) {
        $.ajax(url, {
            beforeSend: function (request) {
                $('.modalbody').prepend('<div class="ajaxformloader" style="position:absolute; width:100%; height:100%; background:rgba(255,255,255,.5); text-align:center; left:0px; top:0px; z-index:2;">Loading</div>');
    ...
    init: function () {
        ...
        pt = $('.hero_single.version_2').css('padding-top');
        if ($('.hero_single.version_2').hasClass('full')) { //alert(parseInt(pt)+' - '+$(window).height());
            hp = (($(window).height() / $(window).width()) * 100);
            if (parseInt(pt) > $(window).height() || ($(window).width() / 100) * 60 > $(window).height()) {
                pt = hp + '%';
            } else {
                pt = '60%';
                if ($(window).width() < 600) {
            ...
            pt = $(window).height() * .5 + 'px';
        }

        $('.hero_single.version_2').css({ paddingTop: pt });
        ...
        itl.r_owl = $('#reccomended');
        itl.r_owl.owlCarousel({
            center: true,
            items: 2,
            loop: true,
            onChanged: function (event) {
                id = $(event.target).find('.owl-item.active.center').find('.item').attr('id');
                // if (id && cr) {
                //     cr.set();
                // }

                if ($('.reccomended-description').length) {
                    setTimeout(function () {
                        desc = $(event.target).find('.owl-item.active.center').find('.item').attr('data-desc');
                        $('.reccomended-description').fadeOut(300, function () {
                            $('.reccomended-description').html(desc);
                            $('.reccomended-description').fadeIn(300);
                        })
                    }, 100);
        ...
        itl.hs_owl = $('#homeservice');
        itl.hs_owl.owlCarousel({
            loop: false,
            margin: 0,
            merge: true,
            nav: true,
            ...
            onInitialized: function (event) {
                console.log(event.relatedTarget.settings.responsiveBaseElement);
                if (event.item.count > 2) {
                    $('#homeservice').siblings('.owl-nav').addClass('active');
                }
            }
        });
    ...
    banner: function () {
        ...
        tmpImg.src = itl.bannerimg[itl.bannerno].main;
        console.log(1);
        tmpImg.onload = function () {
            ele = '<div class="banner-item" style="background-image: url(' + itl.bannerimg[itl.bannerno].main + ')">';
            if (itl.bannerimg[itl.bannerno].svg) {
                ele += '<img src="' + itl.bannerimg[itl.bannerno].svg + '" />';
            }
            ele += '</div>';
...
function menutoggle() {
    if ($('.mheader').hasClass('menu-opened')) {
        ...
        $('header.header').removeClass('m-o');
    } else {
        $('.mheader, .burger-container').addClass('menu-opened');
        $('body').css({'overflow':'hidden'});
        $('header.header').addClass('m-o');
    }
}
...
$(document).on('submit', 'form#ajaxform', function (e) {
    ...
    if (url) {
        ...
        itl.ajaxform(url, formData, modal);
        e.preventDefault();
    }
});
...
$(document).ready(function () {
    ...
    var id = myurl.substring(myurl.lastIndexOf('/') + 1);
    if (id.includes('flight')) {
        $("#pills-flight-tab").click();
    }
    if (id.includes('hotel')) {
        $("#pills-hotel-tab").click();
    }
    ...
    $(".has-child").click(function () {
        if ($(this)) {
            $('li.has-child > ul').not($(this).children("ul").slideToggle("slow")).hide();
...
jQuery(document).ready(function () {
    ...
    $('.enquire').click(function () {
        mp = $(this).parent().parent().attr('data-title');
        $('input[type=checkbox]').each(function () {
            this.checked = false;
        });
        $("input[type=checkbox][value='" + mp + "']").prop("checked", true);
    });

});

$(document).ready(function () {
    $('.custom-accordions-desc .custom-accord:first-child .c-accord-desc-inner').addClass("showdays");
    $('.ca-text-link').click(function () {
	 $(this).toggleClass("adn_color");
        var getParentId = $(this).attr('id');
	// $('.c-accord-desc-inner').removeClass('showdays');

	$('.c-accord-desc-inner').css("display", "none");
        $(this).parents('.accordion-item').find('.' + getParentId).toggleClass('showdays');
...
$(window).on('shown.bs.modal', function(event) { 
    ...
    if (modalId === 'flightModal' || modalId === 'holidayModal' || modalId === 'hotelModal') {
        $('#' + modalId).modal('show');
        $('.upbtn').hide();
    }
});
